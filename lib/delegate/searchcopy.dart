...
// Show the products based on the search query like name, type, price, quantity.
  ...
  buildSearchList3() {
    final AsyncValue<Result> asyncTodo = ref.watch(productControllerProvider);

    return asyncTodo.when(
      loading: () => null,
      error: (error, stack) => print('Oops, something unexpected happened'),
      // ignore: void_checks
      data: (value) {
        List<Product> matchQuery = [];

        final List? productItems = value.items;
